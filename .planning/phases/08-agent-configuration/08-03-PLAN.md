...
must_haves:
  ...
  key_links:
    - from: "app/routes/agents.tsx"
      ...
      pattern: "agentTraits"
    - from: "app/routes/agents.tsx"
      to: "app/components/agent-card.tsx"
      via: "traits prop passing"
      pattern: "traits={userTraits}"
    - from: "app/components/agent-card.tsx"
      to: "app/components/agent-form-dialog.tsx"
      via: "traits prop forwarding"
      pattern: "traits={traits}"
...
**7. Update AgentCard component (app/components/agent-card.tsx):**

Update the AgentCardProps interface to include capability, model, traitIds, and traits:
```typescript
interface AgentCardProps {
  agent: Pick<Agent, "id" | "name" | "instructions" | "updatedAt" | "capability" | "model"> & {
    traitIds?: string[];
  };
...
Update the component to pass traits to AgentFormDialog:
```typescript
...
**8. Update agents.tsx to pass traits to AgentCard:**
In the JSX where agents are mapped:
```typescript
