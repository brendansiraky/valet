...
must_haves:
  truths:
    - "Agent runner uses provider abstraction instead of direct Anthropic SDK"
    - "Pipeline executor uses provider abstraction instead of direct Anthropic SDK"
    - "All existing functionality works identically (web search, web fetch, citations)"
    - "anthropic.server.ts simplified to just validateApiKey (key validation only)"
  artifacts:
...
3. Import from provider layer:
   - `getProviderForModel` from ~/lib/providers/registry
   - Types from ~/lib/providers/types

4. Update runAgent function:
   - Get provider using `getProviderForModel(model)` then create instance with decrypted key
...
Note: Provider.chat() needs the decrypted API key. Update flow:
- Import decrypt from encryption.server
- Decrypt key
- Create provider instance with key: `new AnthropicProvider(decryptedKey)`
- Or use registry factory pattern from Plan 01
...
1. Remove imports from anthropic.server.ts (createAnthropicClient)
2. Remove import of runWithTools
3. Import provider layer: getProviderForModel, types
