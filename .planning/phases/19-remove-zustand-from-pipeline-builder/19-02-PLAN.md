...
6. Compute enriched nodes with orphan status using useMemo (NO useEffect):
```typescript
...
This approach computes isOrphaned during render, avoiding the useEffect anti-pattern entirely. The derived state is recalculated whenever nodes or agents change.
