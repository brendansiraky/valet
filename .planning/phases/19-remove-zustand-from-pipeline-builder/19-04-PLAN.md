...
<task type="auto">
  <name>Task 3: Update pipeline-creation-flow.test.tsx and run full test suite</name>
  <files>app/components/pipeline-builder/pipeline-creation-flow.test.tsx</files>
  <action>
1. Read pipeline-creation-flow.test.tsx and check if it mocks pipeline-store.

2. If it does, update to mock usePipelineFlow instead using the same pattern as above.
...
4. Fix any remaining test failures related to the migration.

5. Verify no test file imports from pipeline-store:
```bash
...
Should return no results.
  ...
  <done>All tests pass, no test files reference pipeline-store</done>
</task>
...
<success_criteria>
1. All test files mock usePipelineFlow instead of usePipelineStore
