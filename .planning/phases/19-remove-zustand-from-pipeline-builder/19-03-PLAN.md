...
must_haves:
  truths:
    ...
    - "TraitNode imports TraitNodeData from use-pipelines instead of pipeline-store"
  ...
  artifacts:
    ...
    - path: "app/components/pipeline-builder/trait-node.tsx"
      provides: "Trait node with correct imports"
      contains: "use-pipelines"
  ...
  key_links:
    ...
    - from: "app/components/pipeline-builder/trait-node.tsx"
      ...
      via: "imports TraitNodeData"
      pattern: "import.*TraitNodeData.*use-pipelines"
...
<objective>
Complete the migration by updating remaining consumers (AgentNode, TraitNode, route) and deleting the Zustand pipeline-store.

Purpose: This plan finalizes the migration. AgentNode needs trait operations routed through React Query. TraitNode needs its import updated. The route file needs cleanup. Finally, pipeline-store.ts is deleted to ensure no regression back to the old pattern.
...
<task type="auto">
  <name>Task 1: Update TraitNode import</name>
  <files>app/components/pipeline-builder/trait-node.tsx</files>
  <action>
Update TraitNode to import TraitNodeData from the correct location:

1. Change import on line 5:
```typescript
// OLD (line 5):
...
This is a simple type-only import change. The component implementation remains unchanged.
  </action>
  <verify>Run `npm run typecheck` - should pass</verify>
  <done>TraitNode imports TraitNodeData from use-pipelines instead of pipeline-store</done>
</task>

<task type="auto">
  <name>Task 2: Update AgentNode to use React Query via context</name>
...
<task type="auto">
  <name>Task 3: Update pipelines.$id.tsx route to remove Zustand dependency</name>
...
<task type="auto">
  <name>Task 4: Delete pipeline-store.ts</name>
...
<success_criteria>
1. TraitNode imports TraitNodeData from use-pipelines
2. AgentNode uses usePipelineFlow for trait operations
3. Route file uses queryClient instead of Zustand
4. pipeline-store.ts is deleted
5. All pipeline operations work correctly
